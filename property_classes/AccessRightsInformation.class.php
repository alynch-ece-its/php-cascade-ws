...
class AccessRightsInformation extends Property
{
	...
	public function __construct( stdClass $ari )
	{
		if( $ari != NULL )
		{
			$this->identifier  = new Identifier( $ari->identifier );
			
			if( isset( $ari->aclEntries->aclEntry ) )
			{
	...
	{
		if( self::DEBUG ){ echo "ARI::L24 Granting read access to " . $g->getName() . BR;  }
	...
	{
		if( self::DEBUG ){ echo "ARI::L31 Granting write access to " . $g->getName() . BR;  }
	...
	{
		if( self::DEBUG ){ echo "ARI::L38 Granting read access to " . $u->getName() . BR;  }
	...
	{
		if( self::DEBUG ){ echo "ARI::L45 Granting write access to " . $u->getName() . BR;  }
