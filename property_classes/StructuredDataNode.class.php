...
class StructuredDataNode extends Property
{
	...
	public function __construct( stdClass $node, DataDefinition $dd, $index, $parent_id='' ) 
	{
		if( $node != NULL ) // $node always a single non-NULL object
		{
			...
			//if( self::DEBUG ) echo "SDN::L33 " . $field_identifier . BR;
			
			$field            = $this->data_definition->getField( $field_identifier );
			//if( self::DEBUG ) { echo "SDN::L36 " . S_PRE; var_dump( $field ); echo E_PRE; }
	...
	public function addChildNode( $node_id )
	{
		...
		if( self::DEBUG ) { echo "SDN::L141 " . "Node ID: " . $node_id . BR . "Field ID: " . $field_id . BR; }
		...
		if( self::DEBUG ) { echo "SDN::L150 " . "Last position: " . $last_pos . BR; }
		
		// create a copy of the last sibling
		$cloned_node = $this->structured_data_nodes[ $last_pos ]->cloneNode();
		if( self::DEBUG ) { echo "SDN::L154 " . S_PRE; var_dump( $cloned_node->toStdClass() ); echo E_PRE; }
	...
	public function cloneNode()
	{
		...
		//if( self::DEBUG ) { echo "SDN::L174 " . S_PRE; var_dump( $this->toStdClass() ); echo E_PRE; }
		if( self::DEBUG ) { echo "SDN::L175 " . "Parent ID: " . $this->parent_id . BR; }
		//if( self::DEBUG ) { echo "SDN::L176 " . S_PRE; var_dump( $this->toStdClass() ); echo E_PRE; }
		
		$clone_obj = new StructuredDataNode( 
			$this->toStdClass(), $this->data_definition, 0, $this->parent_id );
		if( self::DEBUG ) { echo "SDN::L179 " . S_PRE; var_dump( $clone_obj->toStdClass() ); echo E_PRE; }
		
		// work out the new identifier
		$this_identifier       = $this->identifier;
		if( self::DEBUG ) { echo "SDN::L180 " . $this_identifier . BR; }
		...
		if( self::DEBUG ) { echo "SDN::L185 " . $clone_identifier . BR; }
	...
	public function removeLastChildNode( $node_id )
	{
		...
		$field_id = self::getFieldIdentifier( $node_id );
		if( self::DEBUG ) { echo "SDN::L404 " . "Field ID: " . $field_id . BR; }
		if( !$this->data_definition->isMultiple( $field_id ) )
		{
		...
		if( self::DEBUG ) { echo "SDN::L412 " . "First position: " . $first_pos . BR . "Last position: " . $last_pos . BR; }
		$last_node_id = $this->structured_data_nodes[ $last_pos ]->getIdentifier();
		if( self::DEBUG ) { echo "SDN::L414 " . "Last node ID: " . $last_node_id . BR; }
	...
	{
		$child_count = count( $array );
		if( self::DEBUG ) { echo "SDN::L907 Child count: " . $child_count . BR; }
		$shared_id   = self::removeLastIndex( $node_id );
		if( self::DEBUG ) { echo "SDN::L909 Shared ID: " . $shared_id . BR; }
		
		for( $i = $child_count - 1; $i > 0; $i-- )
		{
			if( self::DEBUG ) { echo "SDN::L913 Child ID: " . $array[ $i ]->getIdentifier() . BR; }
			if( strpos( $array[ $i ]->getIdentifier(), $shared_id ) !== false )
			{
				if( self::DEBUG ) { echo "SDN::L916 " . "Found in $i" . BR; }
				break;
			}
		}
		return $i;
	...
	public static function processStructuredDataNodes( $parent_id, &$node_array, $node_std, $data_definition )
	{
		if( self::DEBUG ) { echo "SDN::L925 " . "Parent ID: " . $parent_id . BR; }
		...
		if( self::DEBUG ) { echo "SDN::L929 " . "Node count: " . $node_count . BR; }
		...
		for( $i = 0; $i < $node_count; $i++ )
		{
			...
			// a multiple text or group
			if( $is_multiple )
			{
				if( $previous_identifier == $current_identifier ) // an old one, keep counting
				{
					$cur_index++;
				}
			...
			if( $parent_id != '' )
			{
				$n = new StructuredDataNode( $node_std[$i], $data_definition, $cur_index, $parent_id );
			}
			else
			{
				$n = new StructuredDataNode( $node_std[$i], $data_definition, $cur_index );
			}
